...
void main() {
    ...
    vec4 testColor = texelFetch(Sampler0, ivec2(uv - corners[(gl_VertexID + 3) % 4]), 0);
    vec4 testColor2 = texelFetch(Sampler0, ivec2(uv - corners[(gl_VertexID + 2) % 4].yx), 0);

    if (round(testColor.a * 255) == 8 || round(testColor2.a * 255) == 8 || round(testColor.a * 255) == 252 || round(testColor2.a * 255) == 252) //Shulker
    {
        if (gl_VertexID / 4 == 5)
        {
            cem = 1;
        }
        else if (gl_VertexID / 4 == 11)
        {
            cem = 2;
        }
        else
